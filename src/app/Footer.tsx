...
export function Footer({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) return <footer className="fixed bottom-3 left-1/2 -translate-x-1/2 z-[700] max-w-[calc(100vw-1.5rem)] whitespace-nowrap overflow-x-auto rounded-full bg-slate-950/85 backdrop-blur px-4 py-2 text-[10px] text-slate-300 shadow-xl"><div className="flex items-center gap-3 [&_a]:hover:text-teal-300">{links}<span>v0.4.0 · 仅供演示，不用于导航</span></div></footer>;
  return <footer className="border-t border-slate-200 bg-white/80 px-4 py-7 text-xs text-slate-500"><div className="max-w-[1320px] mx-auto flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between"><p><strong className="text-slate-700">TransitReach 深圳 v0.4.0</strong> · 前端可达性 Demo，仅供交互研究，不用于导航或公共决策。</p><div className="flex flex-wrap gap-x-4 gap-y-2 [&_a]:hover:text-teal-700">{links}</div></div></footer>;
}
