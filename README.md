...
2026-09-02 在本地开发环境以福田站、60 分钟预算实测：266 站、最多一次换乘的可达范围算法约 `6.0ms`，生成 197 个去重区域。结果会随浏览器与设备性能变化；此数据只用于回归基线，不代表出行时间精度。
...
- 未接入：完整 GTFS 时刻表、真实站内换乘时间、公交、真实步行路网、道路拥堵及实时运营状态
